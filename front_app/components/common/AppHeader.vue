...
const pathIndex = ref<string[]>(['home', 'character'])
...
<template>
  ...
  <header>
    <div class="bg-[#c3bbee] w-full h-full flex flex-col">
      <div class="bg-[#aa55aa] flex justify-between w-full h-[30%] pl-3 pr-3">
        画面遷移先一覧
        <p>
          {{ currentTimeRef }}
        </p>
      ...
      <div
        class="bg-[#44aaaa] flex w-full h-[70%] justify-start items-end pl-3 pb-2 text-l font-semibold"
      >
        <div v-for="(title, index) in pathIndex" :key="index" class="flex">
          <div
            v-if="index !== 0"
